...
import { ApiResponse, Miembro } from '../Utils/Interfaces/MeetingRecords'
...
export const getMiembros = async (): Promise<ApiResponse<Miembro[]>> => {
  try {
    ...
    return { status: 'success', data: await response.json() } as ApiResponse<Miembro[]>
  } catch (error) {
...
// Eliminar un miembro
export const deleteMiembro = async (id: number): Promise<ApiResponse<boolean>> => {
  try {
