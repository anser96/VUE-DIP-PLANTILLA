  ...
  <div class="p-4">
    <div>
      ...
      <div class="flex justify-end mb-4">
        <button @click="assignTask(member)" class="btn btn-primary">Asignar Tarea</button>
      </div>
...
const confirmDelete = () => {
  EliminarAsignacion({ idTarea: taskIdToDelete.value } as Task)
...
const GuardarAsignacion = () => {
  ...
  if (tareaSeleccionada) {
    tareasAsignadas.value.push({
      ...tareaSeleccionada,
      tipoResponsable: currentTask.value.tipoResponsable,
      responsableId: currentTask.value.responsableId,
    })
    currentTask.value = {}
    closeTaskModal()
  }
...
const fetchResponsables = async (event) => {
  try {
    let responseMiembros: ApiResponse<Miembro[]> = []
    let responseInvitado: ApiResponse<Invitado[]> = []
    console.log('Tipo seleccionado:', event.target.value)
    if (event.target.value === 'miembro') {
      responseMiembros = await getMiembros()
      responsableOpciones.value = responseMiembros.map((r) => ({ id: r.idMiembro, nombre: r.nombre } as Responsable))
    } else if (event.target.value === 'invitado') {
      console.log('llego')
      responseInvitado = await getInvitados()
      responsableOpciones.value = responseInvitado.map((r) => ({ id: r.idInvitado, nombre: r.nombre } as Responsable))
    }
  } catch (error) {
